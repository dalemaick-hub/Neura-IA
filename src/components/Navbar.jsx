  ...
  return (
    <header className="w-full fixed top-0 left-0 z-50 bg-background/80 backdrop-blur-xl border-b border-outline-variant">
      <nav className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6 text-on-surface">
        ...
        >
          <span
            className={`block h-0.5 w-5 bg-current transition-all ${open ? "rotate-45 translate-y-2" : ""}`}
          />
          <span
      ...
      {open && (
        <div className="md:hidden bg-background/95 backdrop-blur-xl border-t border-outline-variant px-6 py-4 space-y-3">
          {navLinks.map(({ to, label }) => (
            <Link
              key={to}
              to={to}
            ...
            {user ? (
              <>
                ...
                <Link to="/chat" onClick={() => setOpen(false)}>
                  <button
                    type="button"
                    className="w-full px-5 py-2 rounded-full bg-primary text-on-primary font-semibold text-sm"
                  >
                    💬 Ir al chat
                  </button>
                </Link>
              ...
              <>
                <Link to="/signin" onClick={() => setOpen(false)}>
                  <button
                    type="button"
                    className="w-full px-5 py-2 rounded-full border border-white/15 text-white/80 text-sm"
                  >
                    Iniciar sesión
                  </button>
                </Link>
