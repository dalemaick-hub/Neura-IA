    ...
    <div className={`animate-message-in mb-4 flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div className={`flex max-w-[85%] flex-col gap-2 ${isUser ? "items-end" : "items-start"}`}>
        ...
        {!isUser && (message.actionableAdvice || message.checkInPrompt) && (
          <div
            className="w-full rounded-2xl border border-white/10 px-4 py-3 text-white/85 shadow-[0_20px_60px_-35px_rgba(189,157,255,0.35)]"
            style={{ background: `linear-gradient(135deg, ${emotionMeta.orb}22, rgba(15, 23, 42, 0.45))` }}
          >
            ...
            {message.checkInPrompt && <p className="mt-3 text-sm text-white/70">🔔 {message.checkInPrompt}</p>}
          </div>
        )}
