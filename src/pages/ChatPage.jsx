...
function buildWeeklySummary(moods) {
  ...
  if (recentMoods.length === 0) return 'Aún no hay suficiente información emocional esta semana.'
  ...
  const [dominantMood] = Object.entries(counts).sort((a, b) => b[1] - a[1])[0]
...
export default function ChatPage() {
  const { user } = useAuth()
  // Usa el ID real del usuario como sessionId — persiste entre dispositivos
  ...
  const [userProfile, setUserProfile] = useState({ name: displayName, moods: [] })
  const [isReady, setIsReady] = useState(false)
  const [mode, setMode] = useState('calmado')

  useEffect(() => {
    const savedMessages = loadHistory(sessionId)
    const savedProfile = loadUserProfile(sessionId)
    const lastEmotion = [...savedMessages]
      .reverse()
      .find((m) => m.sender === 'neura' && m.emotion)?.emotion

    setMessages(savedMessages)
    setUserProfile({ ...savedProfile, name: displayName })
    setEmotion(lastEmotion || savedProfile.moods.at(-1) || 'neutral')
    setIsReady(true)
  }, [sessionId])
  ...
  }, [isReady, messages, sessionId])

  useEffect(() => {
    if (!isReady) return
    saveUserProfile(userProfile, sessionId)
  }, [isReady, userProfile, sessionId])

  const sendMessage = async (userMessage) => {
    if (!userMessage?.trim() || !sessionId) return

    const trimmedMessage = userMessage.trim()
    setMessages((prev) => [...prev, { sender: 'user', text: trimmedMessage, timestamp: Date.now() }])
    setLoading(true)
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { sender: 'neura', text: 'Error de conexión con Neura. Inténtalo de nuevo.', timestamp: Date.now() },
      ])
    } finally {
      setLoading(false)
    }
  ...
  const handleClearHistory = async () => {
    if (sessionId) await resetNeuraSession(sessionId)
    clearHistory(sessionId)
    setMessages([])
    setEmotion('neutral')
    setUserProfile({ name: displayName, moods: [] })
  }
  ...
  return (
    <NeuraLayout>
      <section className="relative min-h-screen overflow-hidden bg-gradient-to-br from-[#0a0018] via-[#12002e] to-[#1a003d] px-6 py-10 text-white">
        ...
        <div
          className="pointer-events-none absolute left-1/2 top-28 h-72 w-72 -translate-x-1/2 rounded-full blur-3xl opacity-80"
          style={{ background: emotionMeta.orb, animation: 'pulse 3s ease-in-out infinite' }}
        />
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-col items-center">
          <div className="text-center mt-6">
            ...
            <p className="mt-1 tracking-wide text-purple-300">Hola, {displayName}</p>
          ...
          <div className="mb-6 mt-10">
            <div className="relative flex h-40 w-40 items-center justify-center rounded-full bg-gradient-to-br from-purple-500 via-fuchsia-500 to-blue-500 shadow-[0_0_40px_rgba(150,0,255,0.6)]">
              <div className="absolute inset-2 rounded-full border border-white/15 bg-black/10 backdrop-blur-lg" />
              <span className="relative text-5xl">{emotionMeta.emoji}</span>
              <div
                className="emotion-wave absolute -bottom-3 left-1/2 h-px w-40 -translate-x-1/2"
